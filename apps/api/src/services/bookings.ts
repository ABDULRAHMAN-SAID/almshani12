...
export function createBooking(studentId: number, input: {
  ...
  return db.transaction(() => {
    // باقة؟ نتحقّق ونستهلك حصة
    let pkg: { id: number; remaining: number } | null = null;
    if (input.packagePurchaseId) {
      pkg = q.get<any>(`SELECT id, remaining FROM package_purchases WHERE id = ? AND user_id = ? AND teacher_id = ? AND remaining > 0 AND (expires_at IS NULL OR expires_at > ?)`,
        input.packagePurchaseId, studentId, input.teacherId, nowIso()) ?? null;
    ...
    try {
      info = q.run(
        `INSERT INTO bookings (student_id, teacher_id, subject_id, mode, duration_minutes, starts_at, ends_at, status, price, package_purchase_id, note, expires_at)
         VALUES (?,?,?,?,?,?,?,?,?,?,?,?)`,
    ...
    if (pkg) {
      ...
      notify(input.teacherId, { type: 'booking_confirmed', title: 'حجز جديد', body: `حصة ${input.durationMinutes} دقيقة`, data: { bookingId } });
      return { booking: q.get<BookingRow>('SELECT * FROM bookings WHERE id = ?', bookingId)!, order: null };
    }
    const order = createOrder(studentId, { items: [{ type: 'lesson', id: bookingId }], couponCode: input.couponCode ?? null, meta: { bookingId } });
...
export function completeBooking(bookingId: number) {
  return db.transaction(() => {
    ...
    q.run('UPDATE teacher_profiles SET lessons_count = lessons_count + 1, students_count = (SELECT COUNT(DISTINCT student_id) FROM bookings WHERE teacher_id = ? AND status = ?) WHERE user_id = ?', b.teacher_id, 'completed', b.teacher_id);
