...
import { Page, Badge, DataTable, PersonLink, ROLES, STAFF, ar, useToast, errMsg, useConfirm, useDebounced, can, isSuper, STATUS_TONE, type Me, type Column } from '../ui';
...
function UsersTable({ me, initialRole, initialStatus }: { me: Me; initialRole: string; initialStatus: string }) {
  ...
  const setStatusM = useMutation({ mutationFn: ({ id, status, reason }: { id: number; status: string; reason?: string }) => api.post(`/admin/users/${id}/status`, { status, reason }), onSuccess: (_r, v) => { toast('تم'); qc.invalidateQueries({ queryKey: ['adm-users'] }); qc.invalidateQueries({ queryKey: ['adm-user', v.id] }); }, onError: e => toast(errMsg(e)) });
  ...
  const cols: Column<AdminUserRow>[] = [
    ...
    { key: 'act', label: '', className: 'actions', hide: !can(me, 'admin'), render: u => u.id !== me.id && u.status !== 'deleted' && (isSuper(me) || !u.roles.some(r => STAFF.includes(r))) ? (u.status === 'active' ? <button className="btn danger sm" onClick={() => suspend(u)}>إيقاف</button> : <button className="btn success sm" onClick={() => activate(u)}>تفعيل</button>) : null },
  ];
  return (
    <Page title="المستخدمون" sub="الاسم يفتح صفحة الشخص: الأدوار والمحفظة والمتعلّمون والطلبات والجلسات">
      ...
      <div className="card"><DataTable columns={cols} rows={list.data?.data} meta={list.data?.meta} onPage={setPage} loading={list.isLoading} error={list.error} empty="لا نتائج" /></div>
    </Page>
  );
}
