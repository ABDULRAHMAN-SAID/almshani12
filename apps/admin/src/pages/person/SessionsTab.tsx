...
import { Badge, DataTable, STAFF, isSuper, useToast, errMsg, useConfirm, type Column } from '../../ui';
import type { TabProps } from './types';

/** الجلسات والأجهزة: إنهاء جلسة واحدة أو الكل (مع حذف رموز الإشعارات اختياريّاً) — يُبلَّغ الجهاز فوراً */
export function SessionsTab({ id, me, d }: TabProps) {
  ...
  // نفس قيد الخادم: جلسات حسابات الطاقم لا يُنهيها إلا super_admin
  const manage = isSuper(me) || !d.roles.some(r => STAFF.includes(r.role));
  const cols: Column<SessionRow>[] = [
    ...
    { key: 'act', label: '', className: 'actions', render: r => manage && !r.revoked && new Date(r.expiresAt).getTime() >= Date.now() ? <button className="btn danger sm" disabled={revoke.isPending} onClick={() => one(r)}>إنهاء</button> : null },
  ];
  ...
  return (
    <>
      <div className="card"><div className="row between" style={{ marginBottom: 10 }}><h2 style={{ margin: 0 }}>الجلسات (آخر ٥٠)</h2>{manage ? <button className="btn danger sm" disabled={revoke.isPending} onClick={all}>إنهاء الكل</button> : <span className="badge info">حساب طاقم — للمدير العام</span>}</div><DataTable columns={cols} rows={s.data?.sessions} loading={s.isLoading} error={s.error} rowClass={r => r.revoked ? 'dim' : ''} empty="لا جلسات" /></div>
