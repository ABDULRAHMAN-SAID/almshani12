...
export default function Content() {
  ...
  const decide = useMutation({ mutationFn: (b: { decision: string; reason?: string | null }) => api.post(`/admin/content/${item.entityType}/${item.id}/decision`, { ...b, checklist: checks }), onSuccess: () => { toast('تم'); setItem(null); qc.invalidateQueries({ queryKey: ['adm-content'] }); qc.invalidateQueries({ queryKey: ['overview'] }); }, onError: e => toast(errMsg(e)) });
  ...
  return (
    <Page title="مراجعة المحتوى" sub="لا يُنشر شيء دون مراجعة">
      ...
      {item ? (
        <Modal title={item.title} onClose={() => setItem(null)} footer={<><button className="btn danger" disabled={decide.isPending || !reason.trim()} onClick={() => decide.mutate({ decision: 'rejected', reason })}>رفض (بسبب)</button><button className="btn success" disabled={decide.isPending || !allChecked} onClick={() => decide.mutate({ decision: 'approved', reason: null })}>اعتماد ونشر</button></>}>
          ...
          <Field label="سبب الرفض (يصل للمعلّم)"><textarea value={reason} onChange={e => setReason(e.target.value)} /></Field>
        </Modal>
      ) : null}
    </Page>
  );
