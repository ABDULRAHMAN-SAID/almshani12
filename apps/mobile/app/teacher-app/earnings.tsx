...
export default function Earnings() {
  ...
  const d = q.data;
  const tile = (label: string, value: number, tone: 'primary' | 'success' | 'warning' | 'secondary' = 'primary') => <Card key={label} style={styles.tile}><Text role="caption" tone="secondary">{label}</Text><Text role="h2" tabular tone={tone}>{money(value)}</Text></Card>;
  return (
    <Screen onBack={() => router.back()} title={t('teacherApp.earnings')} loading={q.isLoading} error={q.error} onRetry={() => q.refetch()} refreshing={q.isRefetching} onRefresh={() => q.refetch()}>
      {d ? (
        <View style={styles.wrap}>
          <View style={styles.grid}>{tile(t('teacherApp.gross'), d.gross)}{tile(t('teacherApp.commission'), d.commission, 'secondary')}{tile(t('teacherApp.net'), d.net)}</View>
          <View style={styles.grid}>{tile(t('teacherApp.pending'), d.pending, 'warning')}{tile(t('teacherApp.available'), d.available, 'success')}{tile(t('teacherApp.paid'), d.paid, 'secondary')}</View>
