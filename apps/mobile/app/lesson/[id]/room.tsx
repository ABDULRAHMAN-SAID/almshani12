...
export default function Room() {
  ...
  const st = useRoom();
  const [sheet, setSheet] = useState<null | 'chat' | 'people' | 'board'>(null);
  ...
  const ctl = (icon: IconName, label: string, onPress: () => void, opts: { active?: boolean; danger?: boolean; badge?: number; disabled?: boolean } = {}) => (
    <Pressable key={label} onPress={onPress} disabled={opts.disabled} accessibilityRole="button" accessibilityLabel={label} style={[styles.ctl, opts.active && styles.ctlActive, opts.danger && styles.ctlDanger, opts.disabled && styles.ctlDisabled]}>
      <Icon name={icon} size={22} color={opts.danger ? colors.text.inverse : opts.active ? colors.brand.primary : colors.text.inverse} />
      {opts.badge ? <View style={styles.badge}><Text role="caption" tone="inverse" tabular style={styles.badgeText}>{opts.badge}</Text></View> : null}
      <Text role="caption" tone="inverse" numberOfLines={1} style={styles.ctlLabel}>{label}</Text>
    </Pressable>
  );
    ...
    <SafeAreaView style={styles.safe} edges={['top', 'bottom']}>
      ...
      <View style={styles.top}>
        <View style={styles.flex}>
          <Text role="bodyMedium" tone="inverse" numberOfLines={1}>{b?.subject.name} · {otherName}</Text>
          <Text role="caption" tone="inverse" tabular style={styles.dim}>{formatTime(b!.startsAt)}–{formatTime(b!.endsAt)} · {t('lessons.room.elapsed')} {mmss(elapsed)} · −{mmss(remaining)}</Text>
        </View>
        <View style={[styles.conn, st.connection === 'connected' ? styles.connOk : st.connection === 'reconnecting' ? styles.connWarn : styles.connBad]}>
          <Icon name={st.connection === 'connected' ? 'wifi' : 'wifiOff'} size={14} color={colors.text.inverse} />
          <Text role="caption" tone="inverse">{st.connection === 'connected' ? t('live.connected') : st.connection === 'reconnecting' ? t('lessons.room.reconnecting') : st.connection === 'ended' ? t('lessons.room.ended') : t('live.failed')}</Text>
        </View>
      </View>
      ...
      <View style={styles.stage}>
        ...
        {st.sharing ? <View style={styles.shareBanner}><Icon name="screen" size={14} color={colors.text.inverse} /><Text role="caption" tone="inverse">{t('live.sharing')}</Text></View> : null}
      ...
      {/* أزرار التحكّم */}
      <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.controls} style={styles.controlsBar}>
        ...
        {ctl('hand', t('lessons.room.raiseHand'), room.toggleHand, { active: st.hand })}
        {ctl('people', t('lessons.room.participants'), () => setSheet('people'), { badge: st.participants.length })}
        {ctl('board', t('lessons.room.whiteboard'), () => setSheet('board'), { badge: st.boardOps.length || undefined })}
        {ctl('screen', st.sharing ? t('live.stopSharing') : t('lessons.room.share'), room.toggleShare, { active: st.sharing, disabled: !room.webrtc })}
