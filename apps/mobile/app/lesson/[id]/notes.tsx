...
import { Screen, Text, Button, Input, Chip, Card } from '@/ui';
...
export default function LessonNotes() {
  ...
  const [summary, setSummary] = useState(b.data?.notes?.summary ?? '');
  const [homework, setHomework] = useState(b.data?.notes?.homework ?? '');
  ...
  return (
    <Screen onBack={() => router.back()} title={t('teacherUi.notes')} loading={b.isLoading} error={b.error} onRetry={() => b.refetch()}
      footer={<Button label={t('teacherUi.saveNotes')} icon="send" size="lg" full loading={post.isPending} disabled={!summary.trim() && !homework.trim()} onPress={() => post.mutate({ summary: summary.trim() || null, homework: homework.trim() || null, attachmentFileIds: files.map(f => f.id), suggestNext: false }, { onSuccess: () => router.replace(`/lesson/${bookingId}`) })} />}>
