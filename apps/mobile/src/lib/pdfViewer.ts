/**
 * عارض PDF داخل التطبيق (pdf.js من cdnjs يُحمَّل على الجهاز — الملف يُجلب من رابطنا الموقّع فقط، لا يمرّ بخدمة خارجية).
 * الصفحات تُرسَم على canvas عند الاقتراب منها، وتُبلَّغ الصفحة الحالية لتطبيق الهاتف عبر postMessage.
 */
export function viewerHtml({ url, startPage = 1, maxPages = 0 }: { url: string; startPage?: number; maxPages?: number }): string {
