 ...
 */
export type InlinePdf = { pdfjs: string; worker: string; data: string };
/** inline: النسخة أحادية الملف — pdf.js وملف الكتاب (base64) مضمَّنان في الصفحة نفسها بلا أي طلب شبكة */
export function viewerHtml({ url, startPage = 1, maxPages = 0, assetBase, inline }: { url: string; startPage?: number; maxPages?: number; assetBase: string; inline?: InlinePdf }): string {
  const lib = `${assetBase}/static/pdfjs`;
  const lead = inline ? `<script>${inline.worker}</script><script>${inline.pdfjs}</script>` : `<script src="${lib}/pdf.min.js"></script>`;
  const source = inline ? `{data:Uint8Array.from(atob(${JSON.stringify(inline.data)}),c=>c.charCodeAt(0))}` : `{url:URL_}`;
  const workerSrc = inline ? 'inline' : `${lib}/pdf.worker.min.js`;
...
const pdf=await pdfjsLib.getDocument(${source}).promise;const n=MAX?Math.min(pdf.numPages,MAX):pdf.numPages;
