...
export async function handle(method: string, fullPath: string, body: any, ctx?: { learnerId?: number | null }): Promise<{ status: number; body: any }> {
  ...
  /* المعلّمون */
  if (path === '/teachers' && method === 'GET') { let list = D.student.teachers.slice(); const t = norm(q.q ?? ''); if (t) list = list.filter((x: any) => norm(x.name).includes(t) || norm(x.headline ?? '').includes(t) || x.subjects.some((s: any) => norm(s.name).includes(t))); if (q.subjectId) list = list.filter((x: any) => x.subjects.some((s: any) => s.id === Number(q.subjectId))); if (q.maxPrice) list = list.filter((x: any) => x.priceFrom <= Number(q.maxPrice)); if (q.minRating) list = list.filter((x: any) => x.ratingAvg >= Number(q.minRating)); if (q.mode) list = list.filter((x: any) => x.modes.includes(q.mode)); if (q.minYearsExp) list = list.filter((x: any) => x.yearsExp >= Number(q.minYearsExp)); if (q.language) list = list.filter((x: any) => D.teacherProfiles[x.id]?.languages.includes(q.language)); if (q.gradeId) list = list.filter((x: any) => (D.teacherProfiles[x.id]?.grades ?? []).some((g: any) => g.id === Number(q.gradeId))); if (q.sort === 'price_asc') list.sort((a: any, b: any) => a.priceFrom - b.priceFrom); if (q.sort === 'rating') list.sort((a: any, b: any) => b.ratingAvg - a.ratingAvg); return ok(paginate(list.map(teacherCard))); }
  ...
  // مرفق ملاحظات — نسخة العرض تعيد رابطاً تجريبياً بدل «المسار غير موجود»
  if ((r = m(/^\/bookings\/(\d+)\/notes\/files\/(\d+)$/))) return ok({ url: `demo://attachment/${r[2]}` });
  /* السلة والدفع */
  ...
  /* تطبيق المعلّم */
  if (path === '/teacher/me') { if (S.who !== 'teacher' && !S.teacherApplied) return err(404, 'not_found', 'لم تقدّم طلب انضمام بعد'); if (S.who !== 'teacher') return ok({ verificationStatus: 'pending', monthIncome: 0, todayLessons: 0, upcomingLessons: 0, studentsCount: 0, ratingAvg: 0, availableBalance: 0, booksSold: 0, coursesSold: 0, pendingHomeworkReviews: 0, documents: [], lastDecision: null }); return ok(D.teacher.me); }
  ...
  if (path === '/teacher/earnings') { const held = S.payouts.filter((p: any) => ['pending', 'approved'].includes(p.status) && !D.teacher.earnings.payouts.includes(p)); return ok({ ...D.teacher.earnings, available: money(D.teacher.earnings.available - held.reduce((s: number, p: any) => s + p.amount, 0)), requested: money(S.payouts.filter((p: any) => ['pending', 'approved'].includes(p.status)).reduce((s: number, p: any) => s + p.amount, 0)), paid: money(S.payouts.filter((p: any) => p.status === 'paid').reduce((s: number, p: any) => s + p.amount, 0)), payouts: S.payouts }); }
