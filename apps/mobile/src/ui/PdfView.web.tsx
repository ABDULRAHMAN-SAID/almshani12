...
export const PdfView = forwardRef<PdfViewHandle, PdfViewProps>(function PdfView({ url, startPage, maxPages, onMessage }, ref) {
  ...
  const html = useMemo(() => viewerHtml({ url, startPage, maxPages, assetBase: api.base }), [url, startPage, maxPages]);
  ...
  if (fallback) return <iframe ref={frame} src={`${url}#toolbar=0&navpanes=0`} title="reader" style={{ flex: 1, width: '100%', height: '100%', border: 0, background: '#F1EEE7' }} />;
