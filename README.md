...
## الرابط الثابت للخادم التجريبي

الخادم التجريبي على GitHub Actions يغيّر عنوانه مع كل تشغيل ويتوقف بعد ٥ ساعات، لذلك:
- **رابط ثابت لا يتغيّر:** `https://raw.githack.com/ABDULRAHMAN-SAID/almshani12/claude/digital-education-platform-1w1zvq/docs/go.html` — يقرأ العنوان الحالي ويحوّلك إليه (أضف `?to=get-app` لصفحة تحميل التطبيق، أو `?to=admin` للوحة الإدارة).
- **سجلّ العنوان (JSON):** `.live/server.json` على هذا الفرع (يكتبه التشغيل تلقائياً)، والتطبيق يقرأه بنفسه: زر «الاتصال بالخادم التجريبي» في شاشة البداية والإعدادات، ويصحّح العنوان تلقائياً إذا تغيّر.
- **إعادة التشغيل التلقائي:** عند انتهاء المدة يشغّل الخادم نفسه من جديد بآخر تعديلات الفرع. لإيقاف ذلك أضف متغيّر المستودع `LIVE_SERVER_AUTORESTART=0` (Settings → Variables) أو ألغِ التشغيل من Actions.
- تعليق واحد في PR #1 يُحدَّث بالحالة الحالية (🟢/🔴) بدل تعليق جديد كل مرة.

هذا للتجربة فقط؛ للإنتاج استعمل خادماً دائماً بدومين (القسم التالي عن النشر).
...
3. الأسهل من الهاتف: افتح `<رابط-الخادم>/get-app` في Chrome (صفحة تحميل بالعربية مع الخطوات ورمز QR يخدم الملف من الخادم نفسه). أو افتح رابط GitHub أعلاه مباشرةً.
   إذا ظهرت رسالة من Play Protect اختر «التفاصيل» ثم «التثبيت على أي حال»؛ وإذا قال أندرويد «لم يُثبَّت التطبيق» فاحذف أي نسخة سابقة ثم أعد التثبيت (تغيّر مفتاح التوقيع).
...
التوقيع: بلا أسرار يولّد البناء مفتاحاً خاصاً بالمشروع مرة واحدة ويحفظه في ذاكرة Actions (يبقى ما دام البناء يعمل أسبوعياً على الأقل؛ إن ضاع تُثبَّت النسخة التالية بعد حذف القديمة). للمتجر والثبات الدائم أضف مفتاحك كأسرار: للمتجر أنشئ مفتاحاً مرة واحدة وأضفه كأسرار:
```bash
